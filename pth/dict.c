...
static void _set_cell_value(cell_t* cell, void* val, val_type_t val_type) {
    ...
    switch (val_type)
    {
        ...
        case STR:
            ...
            break;
        default:
            err("Не установленный тип значения.");
            break;
    }
}
/*
...
static cell_t* _create_cell(alpha* key, void* val, val_type_t val_type) {
    ...
    switch (val_type)
    {
        ...
        case STR:
            break;
        default:
            crit("Не установленный тип значения.");
            break;
    }
...
void get_elements(dict_t *dict, char** keys, void* values) {
    ...
    switch (dict->val_type)
    {
        ...
        case INT:
            break;
        case LONG:
            get_elements_long(dict, keys, (long*) values);
            break;
        case FLOAT:
            break;
        case DOUBLE:
...
void get_elements_long(dict_t *dict, char** keys, long* values) {
    size_t index = 0;
    size_t len = 0;
    for (size_t i = 0; i < dict->size; i++) {
        if (dict->data[i])  {
            len = strlen((char*)dict->data[i]->key);
            keys[index] = (char*) alloc(sizeof(char) * (len + 1));
            memcpy(keys[index], dict->data[i]->key, len);
            keys[index][len] = '\0';
            values[index] = *((long*)(dict->data[i]->value));
            index++;
        }
    }    
}
