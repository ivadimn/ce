...
int friendly_summ(int* matrix, int rows, int columns) {
    int sum = 0;
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < columns; j++) {
...
int not_friendly_summ(int* matrix, int rows, int columns) {
    int sum = 0;
    for (int j = 0; j < columns; j++) {
        for (int i = 0; i < rows; i++) {
            sum += *(matrix + i * columns + j);
...
int main(int argc, char** argv) {
    ...
    if (strcmp(operation, "print") == 0) {
        ...
        int sum = friendly_summ(matrix, rows, columns);
        printf("Friendly sum: %d\n", sum);
    }
    else if (strcmp(operation, "not-friendly-sum") == 0) {
        int sum = not_friendly_summ(matrix, rows, columns);
